...
<tr><td><img src="http://'.IMG_DOMAIN.'/har/win2.gif" alt="Побед в Две Башни"> Побед в Две Башни: </td><td align=right>' . $char['arcomage_win'] . '</td></tr>
...
echo'<img src="http://'.IMG_DOMAIN.'/nav/xar.gif" align=right><b>Сейчас у тебя <font color="#ffff00">' . $char['clevel'] . '</font> уровень, для следующего уровня нужно: <font color="#ffff00">'.$new_clevel.'</font> exp';

if ($char['clevel']>=5) check_obnul(1);

if ($char['vsadnik']>0) 
{
	echo'<br /><br /><font color="#ffff00"><b>Ты сидишь на ездовом животном: '.mysql_result(myquery("SELECT nazv FROM game_vsadnik WHERE id=".$char['vsadnik'].""),0,0).'!</b></font>';
	...
	if ($sel_golod!=false AND mysql_num_rows($sel_golod)>0)
	{
		...
		echo '<br />Состояние твоего питомца: <b>'.$state.'</b>';
		echo '<br />Возраст: '.($horse['life_horse']-$horse['life']).' / '.$horse['life_horse'];
	}
}
...
{
	echo '<br /><br /><font color="#ffff00">Сейчас ты обладаешь навыками:<br><br>
	<table border="0">';
