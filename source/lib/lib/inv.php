...
<table width="250" border="2" cellspacing="0" cellpadding="0" align="center" background="http://'.IMG_DOMAIN.'/fonhero.gif" height="300" bordercolor="#666666">
  <tr>
	<td height="295">
	  <table width="227" border="0" cellspacing="0" cellpadding="0" height="292">
		<tr>
		  <td width="10" rowspan="9"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="8" height="80"></td>
		  <td width="48" height="29"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="48" height="35"></td>
		  <td rowspan="9" height="288" width="63"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="63" height="30"></td>
		  <td rowspan="9" height="288" width="49">
			<table width="46" border="0" cellspacing="0" cellpadding="0" height="295">
			  <tr>
				<td height="4"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="49" height="8"></td>
			  </tr>
			  <tr>
				<td id="Шлем" height="50" width="50">';
				if (isset($used_items[6]))
				{
					$it = $used_items[6];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
					$Item->hint($it['id'],1,$str_begin);
					ImageItem($Item->fact['img'],0,$Item->item['kleymo'],"middle","Снять предмет","Снять предмет");
					echo '</a>';       
				}
				else
				{
			  ...
			  <tr>
				<td height="2"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="20" height="2"></td>
			  </tr>
			  <tr>
				<td id="Доспех" height="50" width="50">';
				if (isset($used_items[5]))
				{
					$it = $used_items[5];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
			  ...
			  <tr>
				<td height="2"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="20" height="4"></td>
			  </tr>
			  <tr>
				<td id="Пояс" height="50" width="50">';
				if (isset($used_items[8]))
				{
					$it = $used_items[8];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
		  ...
		  <td rowspan="9" height="288" width="57">
			<table width="55" border="0" cellspacing="0" cellpadding="0" height="295">
			  <tr>
				<td height="127" colspan="2"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="30" height="131"></td>
			  </tr>
			  <tr>
				<td width="5" height="51"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="5" height="27"></td>
				<td id="Щит" width="50" height="50">';
				if (isset($used_items[4]))
				{
					$it = $used_items[4];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
		...
		<tr>
		  <td id="Оружие" width="50" height="50">';
				if (isset($used_items[1]))
				{
					$it = $used_items[1];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
		...
		<tr>
		  <td width="50" height="29"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="20" height="27"></td>
		</tr>
		<tr>
		  <td id="Кольцо" width="50" height="50">';
				if (isset($used_items[2]))
				{
					$it = $used_items[2];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
		...
		<tr>
		  <td width="50" height="6"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="20" height="7"></td>
		</tr>
		<tr>
		  <td id="Артефакт" width="50" height="50">';
				if (isset($used_items[3]))
				{
					$it = $used_items[3];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
		...
		<tr>
		  <td width="50" height="2"><img src="http://'.IMG_DOMAIN.'/nav/x.gif" width="20" height="7"></td>
		</tr>
		<tr>
		  <td id="Магия" width="50" height="50">';
				if (isset($used_items[7]))
				{
					$it = $used_items[7];
					$Item = new Item();
					$str_begin = '<a href="http://'.DOMAIN.'/item.php?inv_option=unequip&id='.$it['id'].''.(($from_house) ? '&house&option='.$option.'' : '').'"';
...
if (!isset($_GET['make_amulet']) OR $from_house OR $from_craft)
{
	QuoteTable('open');
	...
	if (!$from_house AND !$from_craft)
	{
		...
		{
			$is_glava = true;
			$sel_clan_items = myquery("SELECT game_items.id,game_items_factsheet.img,game_items_factsheet.name FROM game_items,game_items_factsheet WHERE game_items.item_id=game_items_factsheet.id AND game_items.kleymo=1 AND game_items.kleymo_id=".$char['clan_id']." AND (game_items.user_id<>".$char['user_id']." OR (game_items.user_id=".$char['user_id']." AND game_items.priznak NOT IN (4,0))) ORDER BY game_items.kleymo_nomer ASC");
		}
		$sel_user_items = myquery("SELECT game_items.id,game_items_factsheet.img,game_items_factsheet.name FROM game_items,game_items_factsheet WHERE game_items.item_id=game_items_factsheet.id AND game_items.kleymo=2 AND game_items.kleymo_id=".$char['user_id']." AND (game_items.user_id<>".$char['user_id']." OR (game_items.user_id=".$char['user_id']." AND game_items.priznak NOT IN (4,0))) ORDER BY game_items.kleymo_nomer ASC");  
...
else
{
	if (isset($_POST['save']))
	{
		...
		$str_id = "'".$str_id."'";
		$minus_weight=0;
		$result_items = myquery("SELECT game_items_factsheet.* FROM game_items,game_items_factsheet WHERE game_items.user_id=$user_id AND game_items.used=0 AND game_items.priznak=0 and game_items_factsheet.type=14 AND game_items.item_id=game_items_factsheet.id AND game_items.id IN (".$str_id.") ORDER BY RAND()");
	...
	{
		QuoteTable('open');
		echo '<center><br /><b>Создание амулета из частей:</b><br /></center><br />Отметь нужные части для сборки амулета (не менее 7 частей!):<br />';
		$result_items = myquery("SELECT game_items.id FROM game_items,game_items_factsheet WHERE game_items.user_id=$user_id AND game_items.used=0 AND game_items.priznak=0 and game_items_factsheet.type=14 AND game_items.item_id=game_items_factsheet.id ORDER BY game_items_factsheet.name");
...
elseif (!$from_craft)
{
	...
	echo'
			...
			<table cellpadding="2" cellspacing="0" border="0" width=100%>
				<tr>
					<td><img src="http://'.IMG_DOMAIN.'/har/s.jpg" alt="Сила"> Сила: </td><td align=right>' . $char['STR'] . ''; if ($char['STR']>$char['STR_MAX']) echo '(+'.($char['STR']-$char['STR_MAX']).')'; echo '</td>
				</tr>
				<tr>
					<td><img src="http://'.IMG_DOMAIN.'/har/i.jpg" alt="Интеллект"> Интеллект: </td><td align=right>' . $char['NTL'] . ''; if ($char['NTL']>$char['NTL_MAX']) echo '(+'.($char['NTL']-$char['NTL_MAX']).')'; echo '</td>
				</tr>
				<tr>
					<td><img src="http://'.IMG_DOMAIN.'/har/l.jpg" alt="Ловкость"> Ловкость: </td><td align=right>' . $char['PIE'] . ''; if ($char['PIE']>$char['PIE_MAX']) echo '(+'.($char['PIE']-$char['PIE_MAX']).')'; echo '</td>
				</tr>
				<tr>
					<td><img src="http://'.IMG_DOMAIN.'/har/z.jpg" alt="Защита"> Защита: </td><td align=right>' . $char['VIT'] . ''; if ($char['VIT']>$char['VIT_MAX']) echo '(+'.($char['VIT']-$char['VIT_MAX']).')'; echo '</td>
				</tr>
				<tr>
					<td><img src="http://'.IMG_DOMAIN.'/har/v.jpg" alt="Выносливость"> Выносливость: </td><td align=right>' . $char['DEX'] . ''; if ($char['DEX']>$char['DEX_MAX']) echo '(+'.($char['DEX']-$char['DEX_MAX']).')'; echo '</td>
				</tr>
				<tr>
					<td><img src="http://'.IMG_DOMAIN.'/har/m.jpg" alt="Мудрость"> Мудрость: </td><td align=right>' . $char['SPD'] . ''; if ($char['SPD']>$char['SPD_MAX']) echo '(+'.($char['SPD']-$char['SPD_MAX']).')'; echo '</td>
				</tr>
				<tr>
					<td><img src="http://'.IMG_DOMAIN.'/har/u.jpg" alt="Удача"> Удача: </td><td align=right>' . $char['lucky'] . ''; if ($char['lucky']>$char['lucky_max']) echo '(+'.($char['lucky']-$char['lucky_max']).')'; echo '</td>
				</tr>
	</table>';
