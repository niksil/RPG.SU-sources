	...
	if ($sel!=false and mysql_num_rows($sel)>0 and $broken_instrument!=1 and $est_items==1)
	{
			...
			if (isset($_GET['brevn']) AND isset($_GET['tree']))
			{
				...
				if ($check!=false AND mysql_num_rows($check)>0)
				{
					...
					if ($char['CC']-$char['CW']>=$res['weight'] OR $prov>0)
					{
						...
						myquery("insert into craft_resource_user (user_id, res_id, col) values ($user_id, $id_resource_blok, 1) ON DUPLICATE KEY UPDATE col=col+1");
		...
		{                                  
			...
			else
			{
				...
				function check_obval($tree,&$action1)
				{
					global $char;
				...
				function break_axe(&$action1)
				{
					...
					if ($cur_uselife<=0)
					{
						...
						//myquery("UPDATE craft_build_stonemason SET user_id=0,end_time=0 WHERE user_id=$user_id");
				...
				if($lumber['state']==80)
				{
					...
					$lumberjacktime = getCraftTimes($user_id,5);
					$char_lumberjack = CraftSpetsTimeToLevel(5,$lumberjacktime+1);
					$end_time = time()+max(120,5*60-$char_lumberjack*10);
